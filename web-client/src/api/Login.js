  ...
  logIn (username, password) {
    return new Promise((resolve, reject) => {
      Api().post('login', {
        ...
        resolve({ identifier: response.data.token, username: response.data.username, activeRole: response.data.role, userID: response.data.user_id })
      }).catch((error) => {
        localStorage.clear()
